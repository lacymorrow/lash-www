...
import { commandExists, isNonInteractive, openUrl, run } from "./utils.js";
...
async function triggerGitHubDeploy(repoSlug: string, cwd: string): Promise<void> {
	const s = p.spinner();
	...
	if (result !== null) {
		...
		p.log.info(`Monitor: ${pc.cyan(`https://github.com/${repoSlug}/actions`)}`);
	} else {
