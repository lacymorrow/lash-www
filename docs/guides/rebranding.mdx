...
- **Colors/theme**: Edit the CSS variables in `src/styles/theme.css` — the `--color-1` through `--color-5` variables control accent colors across the site, and the page-loading progress bar (HolyLoader) automatically uses these via `--loader-color`
