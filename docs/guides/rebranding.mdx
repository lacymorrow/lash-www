...
- **Colors/theme**: Edit the CSS variables in `src/styles/theme.css` — update `--color-1` through `--color-5` for accent colors, and update `--loader-color` to match your brand (e.g. `linear-gradient(90deg, hsl(var(--color-1)), hsl(var(--color-2)), hsl(var(--color-3)), hsl(var(--color-1)))` to derive from your theme colors)
