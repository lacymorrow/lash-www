...
└── (cms)/                 # Payload CMS catch-all — [...slug] renders CMS pages
```
...
3. The `(cms)` catch-all `[...slug]` is lower priority than static routes, so it won't conflict at build time — but a static page shadows the CMS page at the same URL.
