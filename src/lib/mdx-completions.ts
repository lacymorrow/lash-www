...
type MDXCompletionItem = Omit<languages.CompletionItem, "range"> & {
...
export const MDX_COMPLETIONS: MDXCompletionItem[] = [
	...
	// Common MDX Patterns
	{
		label: "callout",
		kind: languages.CompletionItemKind.Snippet,
		insertText: ['<Callout type="${1|info,warning,error|}">', "  ${2:content}", "</Callout>"].join(
			"\n"
		),
	...
	},
	{
		...
		kind: languages.CompletionItemKind.Snippet,
		insertText:
			'<Image src="${1:path}" alt="${2:description}" ${3:width={${4:width}}} ${5:height={${6:height}}} />',
