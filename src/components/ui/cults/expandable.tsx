	...
	(
		...
		return (
			...
			<motion.div
				...
				transition={{ duration: transitionDuration, ease: easeType as Easing }}
				{...props}
			>
...
const ExpandableCard = React.forwardRef<HTMLDivElement, ExpandableCardProps>(
	(
		...
		useEffect(() => {
			if (isExpanded) {
				animatedWidth.set(expandedSize.width || width || 0);
				animatedHeight.set(expandedSize.height || height || 0);
			} else {
