...
export default async function AdminLayout({
	...
	const userIsAdmin = session && 'user' in session ? await isAdmin({ email: session?.user?.email }) : false;

	if (!userIsAdmin) {
		console.warn("User is not an admin, redirecting to home", session && 'user' in session ? session?.user?.email : 'no user');
